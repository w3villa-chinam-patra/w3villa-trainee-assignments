...
function FavoriteCard({ favorite }) {
      ...
      dispatch(
        setUser({
          ...
          favorites: user.favorites.filter(
            ({ id, category }) => Number(favorite.id) !== id
          ),
        })
      );
  ...
  return (
    <Link to={`${MOVIE_DETAILS_ROUTE}/${favorite.category}/${data?.id}`}>
      <div className="transition-all relative duration-200 flex flex-col rounded-2xl cursor-pointer p-3 max-w-[250px] mx-auto">
        ...
        <div
          ...
          className="favorite-icon absolute  flex justify-center items-center bg-neutral-800/60 border border-neutral-500 text-white text-2xl rounded-full right-0 top-0 p-1.5"
        >
